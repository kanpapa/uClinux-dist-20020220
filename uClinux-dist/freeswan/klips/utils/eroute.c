...
char eroute_c_version[] = "RCSID $Id: eroute.c,v 1.43 2001/06/15 05:02:05 rgb Exp $";
...
usage(char* arg)
{
	fprintf(stdout, "usage: %s --{add,replace} --eraf <inet | inet6> --src <src>/<srcmaskbits>|<srcmask> --dst <dst>/<dstmaskbits>|<dstmask> <SA>\n", arg);
	fprintf(stdout, "            where <SA> is '--af <inet | inet6> --edst <edst> --spi <spi> --proto <proto>' OR '--said <said>' OR '--said <%%passthrough | %%passthrough4 | %%passthrough6 | %%drop | %%reject | %%trap | %%hold | %%pass>'.\n");
...
/*	int ret; */
	...
	while((c = getopt_long(argc, argv, ""/*"acdD:e:i:hprs:S:f:vl:+:g"*/, longopts, 0)) != EOF) {
		...
		case 'e':
			...
			if((error_s = ttoaddr(optarg, 0, said_af, &said.dst)) != 0) {
				if(error_s) {
		...
		case 'D':
			if(dst_opt) {
				fprintf(stderr, "%s: Error, DDST parameter redefined:%s, already defined as:%s\n",
					program_name, optarg, dst_opt);
				exit (1);
			}				
			if (ttosubnet(optarg, 0, eroute_af, &d_subnet)) {
		...
		case 'S':
			if(src_opt) {
				fprintf(stderr, "%s: Error, SDST parameter redefined:%s, already defined as:%s\n",
					program_name, optarg, src_opt);
				exit (1);
			}				
			if (ttosubnet(optarg, 0, eroute_af, &s_subnet)) {
