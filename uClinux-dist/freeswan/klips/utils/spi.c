...
char spi_c_version[] = "RCSID $Id: spi.c,v 1.74 2001/06/14 19:35:14 rgb Exp $";
...
/*	int ret; */
	...
	while((c = getopt_long(argc, argv, ""/*"H:P:Z:46dcA:E:e:s:a:w:i:D:S:hvgl:+:"*/, longopts, 0)) != EOF) {
		switch(c) {
		case 'g':
			debug = 1;
			pfkey_lib_debug = 1;
		...
		case 'e':
			...
			if((error_s = ttoaddr(optarg, 0, address_family, &edst)) != 0) {
				if(error_s) {
		...
		case 'I':
			...
			if((error_s = ttosa(optarg, 0, &said)) != NULL) {
			...
			if(address_family != addrtypeof(&said.dst)) {
		...
		case 'D':
			...
			if((error_s = ttoaddr(optarg, 0, address_family, &dst)) != 0) {
				if(error_s) {
		...
		case 'S':
			...
			if((error_s = ttoaddr(optarg, 0, address_family, &src)) != 0) {
				if(error_s) {
	...
	/* for registering SA types that can be negociated */
	/* Register the fact that I can negociate AH SA's */
	...
	/* Register the fact that I can negociate ESP SA's */
	pfkey_extensions_init(extensions);
	if((error = pfkey_msg_hdr_build(&extensions[0],
					SADB_REGISTER,
					SADB_SATYPE_ESP,
	...
	/* Register the fact that I can negociate IPIP SA's */
	pfkey_extensions_init(extensions);
	if((error = pfkey_msg_hdr_build(&extensions[0],
					SADB_REGISTER,
					SADB_X_SATYPE_IPIP,
	...
	/* Register the fact that I can negociate COMP SA's */
	pfkey_extensions_init(extensions);
	if((error = pfkey_msg_hdr_build(&extensions[0],
					SADB_REGISTER,
					SADB_X_SATYPE_COMP,
...
#ifdef PFKEY_LIFETIME
		if((error = pfkey_lifetime_build(&extension[SADB_EXT_LIFETIME_SOFT],
