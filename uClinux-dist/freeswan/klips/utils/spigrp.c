...
char spigrp_c_version[] = "RCSID $Id: spigrp.c,v 1.38 2001/06/14 19:35:15 rgb Exp $";
...
main(int argc, char **argv)
{
	...
	for(i = 0; i < nspis; i++) {
		...
		if(said_opt) {
			if((error_s = ttosa((const char *)argv[i+2], 0, (ip_said*)&(said_af_array[i].said))) != NULL) {
			...
			if((error_s = ttoaddr(argv[i*4+2], 0, said_af_array[i].af, &(said_af_array[i].said.dst))) != 0) {
				if(error_s) {
