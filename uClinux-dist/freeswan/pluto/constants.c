 ...
 * for more details.
 *
 * RCSID $Id: constants.c,v 1.47 2001/05/29 18:39:56 dhr Exp $
 */
...
enum_names oakley_auth_names =
    { OAKLEY_PRESHARED_KEY, OAKLEY_ELGAMAL_ENC_REV, oakley_auth_name, NULL };
...
void init_constants(void)
{
    ...
    ugh = anyaddr(AF_INET, &ipv4_any);
    passert(ugh == NULL);
    ugh = anyaddr(AF_INET, &ipv6_any);
    passert(ugh == NULL);

    ugh = rangetosubnet(&ipv4_any, &ipv4_any, &ipv4_wildcard);
    passert(ugh == NULL);
    ugh = rangetosubnet(&ipv6_any, &ipv6_any, &ipv6_wildcard);
    passert(ugh == NULL);

    ugh = initsubnet(&ipv4_any, 0, '0', &ipv4_all);
    passert(ugh == NULL);
    ugh = initsubnet(&ipv6_any, 0, '0', &ipv6_all);
    passert(ugh == NULL);
}
