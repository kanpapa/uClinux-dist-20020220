 ...
 * for more details.
 *
 * RCSID $Id: constants.h,v 1.68 2001/05/29 18:39:56 dhr Exp $
 */
...
#define DBG_KLIPS	0x20	/* messages to KLIPS */
#define DBG_PRIVATE	0x40	/* private information: DANGER! */

#define DBG_NONE	0	/* no options on */
#define DBG_ALL		0x3f	/* all options on EXCEPT DBG_PRIVATE */
#endif
 ...
 *
 * STATE_MAIN_R0 and STATE_QUICK_R0 are intermediate state (not
 * retained between messages) representing the state when the first
 * message of an exchange has been read but not processed.  The name
 * reflects the fiction that a zeroth responder message has been sent.
 *
 * STATE_MAIN_I4 is an exception because there is no such message.
 * It is best to think of this as a notional empty message following
 * the receipt of STATE_MAIN_R3 message.
 *
 * Similarly, there is no STATE_QUICK_R2 message, but the state
 * represents having received the STATE_QUICK_I2 message.
 *
 * state                 routine that sets it
 * =====                 ====================
 * STATE_MAIN_R0       comm_handle: Main Mode packet but no state
 * STATE_MAIN_I1       main_outI1
 * STATE_MAIN_R1       main_inI1_outR1
 * STATE_MAIN_I2       main_inR1_outI2
 * STATE_MAIN_R2       main_inI2_outR2
 * STATE_MAIN_I3       main_inR2_outI3
 * STATE_MAIN_R3       main_inI3_outR3
 * STATE_MAIN_I4       main_inR3
 *
 * STATE_QUICK_R0      comm_handle: Quick Mode packet no Quick Mode state
 * STATE_QUICK_I1      quick_outI1
 * STATE_QUICK_R1      quick_inI1_outR1
 * STATE_QUICK_I2      quick_inR1_outI2
 * STATE_QUICK_R2      quick_inI2
 */
...
typedef const char *complaint_t;	/* NULL iff happy */
