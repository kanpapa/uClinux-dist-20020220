 ...
 * for more details.
 *
 * RCSID $Id: kernel.c,v 1.128 2001/06/16 22:02:39 dhr Exp $
 */
...
process_pfkey_acquire(pfkey_buf *buf, struct sadb_ext *extensions[SADB_EXT_MAX + 1])
{
    ...
    if (buf->msg.sadb_msg_pid != 0)
	return;	/* we only wish to hear from kernel */
    ...
    ugh = rangetosubnet(&src, &src, &ours);
    passert(ugh == NULL);

    ugh = rangetosubnet(&dst, &dst, &his);
    passert(ugh == NULL);
...
# define NL " "
    ...
    if (!no_klips)
    {
    	const char *temp_name = "/tmp/pluto_script";
	int fd;
...
could_route(struct connection *c)
{
    ...
    {
	loglog(RC_ROUTE, "cannot route connection without knowing nexthop");
	return FALSE;
    }
    ...
    if (ero != NULL && ero != c)
    {
	loglog(RC_LOG_SERIOUS, "cannot install eroute -- it is in use for \"%s\""
	    , ero->name);
	return FALSE;	/* another connection already using the eroute */
    }
#endif /* KLIPS */
...
unroute_connection(struct connection *c)
{
	...
	(void) do_command(c, "unroute");
    	...
    	no_active_tunnels--;
	/* 
	 *  Turn off the VPN led if no more connections are active
	 */
	if (no_active_tunnels == 0) {
 ...
 * Trick: return a pointer to the entry; this allows the entry to be deleted.
 */
...
, unsigned int op, const char *opname UNUSED)
{
    ...
    return pfkey_msg_start(op & ~ERO_REPLACE_FLAG, satype
...
, bool repl, const char *opname)
{
    ...
    passert(addrtypeof(src) == addrtypeof(dst));
    ugh = rangetosubnet(src, src, &this_client);
    passert(ugh == NULL);
    ugh = rangetosubnet(dst, dst, &that_client);
    passert(ugh == NULL);
...
, const ip_address *src, const ip_address *dst)
{
	...
	if (rn != ro)
	{
	    ...
	    ? !eroute_connection(c, htonl(SPI_HOLD), SA_INT, SADB_X_SATYPE_INT
	        , ERO_REPLACE, "replace broader HOLD")
	    : !eroute_connection(c, htonl(SPI_HOLD), SA_INT, SADB_X_SATYPE_INT
	        , ERO_ADD, "add broader HOLD"))
	    {
		return FALSE;
	    }
	}
...
setup_half_ipsec_sa(struct state *st, bool inbound)
{
	...
	*said_next = said;
	...
	&& finish_pfkey_msg(extensions, "Add outgoing IPIP SA", text_said, NULL)))
 ...
 */
static bool
route_and_eroute(struct connection *c, struct state *st)
{
#ifdef KLIPS
    struct connection *ero	/* who, if anyone, owns our eroute? */
