 ...
 * for more details.
 *
 * RCSID $Id: server.c,v 1.65 2001/06/16 16:31:41 dhr Exp $
 */
...
int select_found = NULL_FD;	/* FD of interface that select found ready */
...
find_raw_ifaces4(void)
{
	...
	ugh = initaddr((const void *)&rs->sin_addr, sizeof(struct in_addr)
	    , AF_INET, &ri.addr);
	passert(ugh == NULL);
...
find_raw_ifaces6(void)
{
	...
	for (;;)
	{
	    ...
	    unsigned int scope;	/* proc field, not used */
	    ...
	    ugh = ttoaddr(sb, 0, AF_INET6, &ri.addr);
	    passert(ugh == NULL);
...
process_raw_ifaces(struct raw_iface *rifaces)
{
	 ...
	 */
	{
		...
		if (q == NULL)
		{
		    ...
		    {
			log_errno((e, "bind() for %s:%u in process_raw_ifaces()"
...
call_server(void)
{
	...
	else
	{
		...
		if (FD_ISSET(ifp->fd, &readfds))
		{
		    ...
		    /* Fudge to avoid recvfrom hang.
		     * There is no known reason why this should be needed.
			...
			if (nndes < 0)
			{
			    log_errno((e, "re-select() failed in call_server"));
			    ndes--;
			    continue;
			}
			if (nndes == 0)
			{
			    log("SURPRISE: re-select() in call_server finds %s no longer ready for input"
				, ifp->rname);
			    ndes--;
			    continue;
			}
