 ...
 * for more details.
 *
 * RCSID $Id: spdb.c,v 1.69 2001/04/09 19:48:43 dhr Exp $
 */
...
, enum_names **attr_val_descs UNUSED
, pb_stream *pbs)
{
    ...
    struct state *st)	/* current state object */
{
	...
	while (pbs_left(&trans_pbs) != 0)
	{
		...
		{
		    loglog(RC_LOG_SERIOUS, "invalid value %u for attribute %s in Oakley Transform"
		...
		case OAKLEY_AUTHENTICATION_METHOD | ISAKMP_ATTR_AF_TV:
		    ...
		    /* default policy is to accept any method */
		    if (iap == LEMPTY)
			iap = POLICY_ID_AUTH_MASK;
		...
		case OAKLEY_LIFE_TYPE | ISAKMP_ATTR_AF_TV:
		    ...
		    {
			loglog(RC_LOG_SERIOUS, "attribute OAKLEY_LIFE_TYPE value %s repeated"
...
init_st_oakley(struct state *st, lset_t policy)
{
    ...
    ta.encrypter = &oakley_encrypter[ta.encrypt];
    ta.hash = oakley_tr_am[1].val;  /* OAKLEY_HASH_ALGORITHM */
    ta.hasher = &oakley_hasher[ta.hash];
