 ...
 * for more details.
 *
 * RCSID $Id: timer.c,v 1.64 2001/05/28 19:18:20 dhr Exp $
 */
...
event_handle(void)
{
	...
	case EVENT_RETRANSMIT:
		...
		&& st->st_connection->sa_keying_tries == 0
		...
		else
		{
		    ...
		    {
		    case STATE_MAIN_I3:
			details = ".  Possible authentication failure: no acceptable"
			    " response to our first encrypted message";
			break;
		    case STATE_MAIN_I1:
			details = ".  No acceptable response to our first IKE message";
			break;
		    default:
			break;
		    }
	...
	case EVENT_SA_EXPIRE:
		...
		{
		    satype = "ISAKMP";
		    latest = st->st_connection->newest_isakmp_sa;
		}
		else
		{
		    satype = "IPsec";
		    latest = st->st_connection->newest_ipsec_sa;
		}

		if (st->st_serialno == latest)
		    log("%s SA expired (LATEST!)", satype);
		else
		    log("%s SA expired (superseded by #%lu)", satype, latest);
	    }
	    /* FALLTHROUGH */
