...
static void dragen2_init_IRQ(void)
{
	int i;
...
static int dragen2_request_irq(unsigned int irq, void (*handler)(int, void *, struct pt_regs *),
...
static void dragen2_free_irq(unsigned int irq, void *dev_id)
{
	if (irq >= INTERNAL_IRQS) {
...
static void dragen2_enable_irq(unsigned int irq)
{
	if (irq >= INTERNAL_IRQS) {
		printk("%s: Unknown IRQ %d\n", __FUNCTION__, irq);
		return;
...
static void dragen2_disable_irq(unsigned int irq)
{
	if (irq >= INTERNAL_IRQS) {
		printk("%s: Unknown IRQ %d\n", __FUNCTION__, irq);
		return;
 ...
 */
static void dragen2_do_irq(int vec, struct pt_regs *fp)
{
	int irq;
	int mask;
...
static int dragen2_get_irq_list(char *buf)
{
	int i, len = 0;
	irq_node_t *node;
...
void config_dragen2_irq(void)
{
	mach_default_handler = NULL;
	mach_init_IRQ        = dragen2_init_IRQ;
	mach_request_irq     = dragen2_request_irq;
	mach_free_irq        = dragen2_free_irq;
	mach_enable_irq      = dragen2_enable_irq;
	mach_disable_irq     = dragen2_disable_irq;
	mach_get_irq_list    = dragen2_get_irq_list;
	mach_process_int     = dragen2_do_irq;
}
