	...
	for (init = inits; init->name; init++) {
		...
		for (i = 0; i < init->num; i++) {
			...
			/* Wherever we stopped looking is one past the
			   GINT we want. */
			gbus_int_gint[init->base + i - GBUS_INT_BASE_IRQ]
...
static char gint_names[NUM_USED_GINTS][6];
...
__init struct gbus_int_irq_init gbus_irq_inits[] = {
	/* First set defaults.  */
	{ "GBUS_INT", IRQ_GBUS_INT(0), IRQ_GBUS_INT_NUM,  6},
	{ 0 }
};
...
void __init gbus_int_init_irqs (void)
{
	...
	for (i = 0; i < NUM_USED_GINTS; i++) {
		...
		gint_names[i][2] = 'N';
		gint_names[i][3] = 'T';
		gint_names[i][4] = '0' + gint;
		gint_names[i][5] = '\0';

		/* We initialize one GINT interrupt at a time.  */
		gint_irq_init[0].name = gint_names[i];
