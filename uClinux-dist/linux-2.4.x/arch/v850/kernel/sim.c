...
/* The (arbitrary) size of RAM for our simulated machine.  */
#define MEM_SIZE ((long)(8*1024*1024))
/* The name of a file containing the root filesystem.  */
#define ROOT_FS "blkmem0.image"
...
/* These are where `blkmem' device 0 is allocated.  We put the root
   filesystem there.  */
unsigned long blkmem_0_addr, blkmem_0_len;
...
void __init mach_setup (char **cmdline)
{
	...
	printk ("CPU: NEC v850e (GDB simulator)\n");

	simcons_setup ();

	printk (KERN_INFO "Reading root filesystem: %s", ROOT_FS);
	if (read_file (ROOT_FS, &blkmem_0_addr, &blkmem_0_len, &err)) {
		printk (" (size %luK)\n", blkmem_0_len / 1024);
		...
		blkmem_0_addr = (unsigned long)bogus_blkmem_array;
		blkmem_0_len = 0;
	}
}
...
{
	init_irq_handlers (0, NUM_MACH_IRQS, &sim_irq_type);
}
