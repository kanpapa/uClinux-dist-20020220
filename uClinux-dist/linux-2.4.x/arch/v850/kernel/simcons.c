...
static void simcons_write (struct console *co, const char *s, unsigned count)
{
	V850_SIM_SYSCALL (write, 1, s, count);
}

static int simcons_wait_key (struct console *co)
{
	char buf[1];
	int len = V850_SIM_SYSCALL (read, 0, &buf, 1);
	if (len == 1)
		return buf[0];
	else
		return -1;
...
void simcons_setup (void)
{
	...
	printk ("Console: GDB/v850e simulator stdio\n");
}
