...
static struct mtd_partition keywest_partitions[] = {
	{
		name: "KeyWest bootloader",
		offset: 0x00000000,
		size:   0x00080000
	},
	{
		name: "KeyWest bootargs",
		offset: 0x00080000,
		size:   0x00080000
	},
	{
	...
	{
		name: "KeyWest kernel/root",
		offset: 0x00200000,
		size:   0x00e00000
	},
	{
		name: "KeyWest Intel flash",
		offset: 0
	}
};
...
static int __init keywest_probe(unsigned long addr, int buswidth)
{
	...
	if(!mymtd)
		mymtd = do_map_probe("map_ram", &keywest_map);
	if(!mymtd)
		mymtd = do_map_probe("map_rom", &keywest_map);
