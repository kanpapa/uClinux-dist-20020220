...
#define __KSTK_TOS(task) ((unsigned long)(task) + KERNEL_STACK_SIZE)
#define __KSTK_REGS(task) ((struct pt_regs *)__KSTK_TOS(task) - 1)
#define KSTK_EIP(task)	(__KSTK_REGS(task)->pc)
#define KSTK_ESP(task)	(__KSTK_REGS(task)->gpr[GPR_SP])
