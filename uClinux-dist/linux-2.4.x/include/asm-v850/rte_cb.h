...
#define GBUS_INT_BASE_IRQ	NUM_CHIP_IRQS
#define GBUS_INT_BASE_ADDR	(GCS2_ADDR + 0x00006000)
#include <asm/gbus_int.h>

/* We redefine NUM_MACH_IRQS to include extra interrupts from the GBUS.  */
#undef NUM_MACH_IRQS
#define NUM_MACH_IRQS		(NUM_CHIP_IRQS + IRQ_GBUS_INT_NUM)
...
#define FIXUP_ARENAS							      \
	...
	arena[0].address = (unsigned long)&_blkmem_0_start;		      \
	arena[0].length = &_blkmem_0_end - &_blkmem_0_start;
