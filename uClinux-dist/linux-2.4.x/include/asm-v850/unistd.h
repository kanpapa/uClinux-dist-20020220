...
#define SYSCALL_CLOBBERS	"r1", "r5", "r11", "r12", "r13", "r14",       \
				"r15", "r16", "r17", "r18", "r19"
...
{									      \
  ...
  __asm__ __volatile__ ("mov %1, " SYSCALL_NUM ";"			      \
			"trap " SYSCALL_SHORT_TRAP			      \
			: "=r" (__ret)					      \
			: "i" (__NR_##name)				      \
			: SYSCALL_NUM, SYSCALL_CLOBBERS);		      \
...
{									      \
  ...
  __asm__ __volatile__ ("mov %1, " SYSCALL_NUM ";"			      \
			"trap " SYSCALL_SHORT_TRAP			      \
			: "=r" (__ret)					      \
			: "i" (__NR_##name), "r" (__a), "r" (__b), "r" (__c)  \
			: SYSCALL_NUM, SYSCALL_CLOBBERS);		      \
  __syscall_return (type, __ret);					      \
}
