...
void __init kmem_cache_sizes_init(void)
{
	...
	do {
		...
		sprintf(name,"size-%Zd", (unsigned long) sizes->cs_size);
		if (!(sizes->cs_cachep =
		...
		sprintf(name, "size-%Zd(DMA)", (unsigned long) sizes->cs_size);
...
void kfree (const void *objp)
{
	...
	if (!PageSlab(p)) {
		...
		if ((p->index & ~0x1ff) == 0xa110c200)
			free_contiguous_pages(objp, p->index & 0x1ff);
		else
			BUG();
		local_irq_restore(flags);
		return;
...
#ifdef CONFIG_CONTIGUOUS_PAGE_ALLOC
		if ((page->index & ~0x1ff) == 0xa110c200)
			size = (page->index & 0x1ff) * PAGE_SIZE;
		else
#endif
		{
