...
{
    unsigned int e8390_base = dev->base_addr;
    struct ei_device *ei_local = (struct ei_device *) dev->priv;
...
{
    struct device *dev;
    unsigned int e8390_base;
    int interrupts, nr_serviced = 0;
    struct ei_device *ei_local;

    dev = (struct device *)(irq2dev_map[irq & 0xf]);
    ...
    while ((interrupts = inb_p(e8390_base + EN0_ISR)) != 0
		...
		if (interrupts & (ENISR_TX|ENISR_TX_ERR)) {
			ledman_cmd(LEDMAN_CMD_SET,
					ei_local->interface_num ? LEDMAN_LAN2_TX : LEDMAN_LAN1_TX);
		} else {
			ledman_cmd(LEDMAN_CMD_SET,
					ei_local->interface_num ? LEDMAN_LAN2_RX : LEDMAN_LAN1_RX);
		}
#endif
