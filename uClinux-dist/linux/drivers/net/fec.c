 ...
 */
#if defined(CONFIG_NETtel) || defined(CONFIG_MARCONINAP)
unsigned char   fec_defethaddr[] = { 0x00, 0xd0, 0xcf, 0x00, 0x00, 0x01 };
#endif /* CONFIG_NETtel || CONFIG_MARCONINAP*/
...
static int	mii_queue(int request, void (*func)(uint));
...
static int
mii_queue(int regval, void (*func)(uint))
{
	unsigned long	flags;
	mii_list_t	*mip;
	int		retval;
...
{
	if (((mii_reg >> 18) & 0x1f) == 0x11) {
		/* status register.
		*/
		printk("fec: link %s", (mii_reg & 0x0400) ? "up" : "down");
		printk(", %d Mbps", (mii_reg & 0x4000) ? 100 : 10);
		printk(", %s-Duplex", (mii_reg & 0x0200) ? "Full" : "Half");
		printk("%s\n", (mii_reg & 0x0080) ? ", auto complete" : "");
		/*
		 * adjust a couple of settings if we can
		 */
		if (mii_reg & 0x0200) { /* FULL Duplex */
			fec_hwp->fec_r_cntrl &= ~0x0002; /* rcv on TX */
			fec_hwp->fec_x_cntrl |= 0x0004; /* FDPLX */
		} else {
			fec_hwp->fec_r_cntrl |= 0x0002;  /* !rcv on TX */
			fec_hwp->fec_x_cntrl &= ~0x0004; /* !FDPLX */
		}
...
static void
mii_phyid(uint mii_reg)
{
	static unsigned int	fec_phyid;

	if (((mii_reg >> 18) & 0x1f) == 0x02) {
		/* PHY id - part 1 */
		fec_phyid = mii_reg << 16;
		mii_queue(mk_mii_read(0x03), mii_phyid);
	}
	if (((mii_reg >> 18) & 0x1f) == 0x03) {
		/* PHY id - part 2 */
		fec_phyid |= (mii_reg & 0xffff);
		printk("fec: PHY id 0x%08x", fec_phyid);

		if (fec_phyid == 0x0001378e)
			printk(" -- LXT971");
		if (fec_phyid == 0x001378e2) {
			printk(" -- LXT972");
			mii_queue(mk_mii_write(0x14, 0xd422), NULL);
		}
...
mii_startup_cmds(void)
{
	...
	*/
	mii_queue(mk_mii_read(0x02), mii_phyid);
	mii_queue(mk_mii_read(0x11), mii_status);
}
