...
static const char version[] =
    "smc91111.c:v1.1 01/29/02 by Kendrick Hamilton (hamilton@sedsystems.ca)\n";
...
//TODO: add other big endian processors here.
...
//    int failed = 0;
    ...
    do
    {
        ...
        if ((status & PHY_STAT_ANEG_ACK) == 0)
        {
            smc_wait_ms(500); // wait 500 millisecs
            if (signal_pending(current)) // Exit anyway if signaled
            {
                printk(KERN_DEBUG "%s:PHY auto-negotiate interrupted by "
                        "signal\n", dev->name);
                timeout = 0;
            }
            else
    ...
    } while((timeout > 0) && ((status & PHY_STAT_ANEG_ACK) == 0));
    ...
    if (status & PHY_STAT_REM_FLT)
    {
        printk(KERN_DEBUG "%s:PHY auto-negotiate remote fault detected\n",
                dev->name);
        //failed = 1;
    }

    if ((timeout < 1) || (status & PHY_STAT_REM_FLT))
    {
        printk("%s:Setting to 10Mbps, half duplex\n", dev->name);
        smc_write_phy_register(ioaddr, phyaddr, PHY_CNTL_REG, PHY_CNTL_RST);

        timeout = 6;
        while((timeout >= 0) && (smc_read_phy_register(ioaddr, phyaddr,
                            PHY_CNTL_REG) & PHY_CNTL_RST))
        {
            timeout = timeout - 1;
            smc_wait_ms(500);
            if(signal_pending(current))
            {
                printk(KERN_DEBUG"%s:PHY reset interrupted by signal\n",
                        dev->name);
                timeout = 0;
            }
        }
        ...
        {
            //Set to 10 Mbps, half duplex, no autonegotiate
            //smc_write_phy_register(ioaddr, phyaddr, PHY_CNTL_REG, 0);
            smc_write_phy_register(ioaddr, phyaddr, PHY_CNTL_REG, 0);
            ...
            do
            {
                PRINTK3("%s:Reading Physical Status Read Loop %d\n", dev->name,
                        timeout);

                status = smc_read_phy_register(ioaddr, phyaddr, PHY_STAT_REG);
                smc_interpret_phy_register(dev, PHY_STAT_REG, status);
                smc_register_dump(dev);
                ...
                if(signal_pending(current))
                {
                    PRINTK3("%s:Status reads intterrupted by signal\n",
                            dev->name);
                    timeout = 0;
                }
            } while((timeout>=0) && (((status & PHY_STAT_LINK) == 0) ||
                    (status & PHY_STAT_REM_FLT)) );

            if(timeout <= 0)
                PRINTK2("%s:Timed out waiting for 10Mb Half Duplex link to "
                        "become valid\n", dev->name);

            lp->lastPhy18 = smc_read_phy_register(ioaddr, phyaddr, PHY_INT_REG);
            smc_interpret_phy_register(dev, PHY_INT_REG, lp->lastPhy18);
            smc_register_dump(dev);
        } //If timed out on reset
    } //If timed out auto-negotiate or remote fault
