...
#ifdef SERVER_SSL
#define SSL_KEYF "/etc/config/ssl.pem"
#define SSL_CERTF "/etc/config/ssl.pem"
...
int main(int argc, char **argv)
{
	...
	openlog("boa", 0, 0);
#ifdef SERVER_SSL
	while ((c = getopt(argc, argv, "p:vc:dn")) != -1) {
#else
	while ((c = getopt(argc, argv, "p:vc:d")) != -1) {
		...
		default:
#if 0
			fprintf(stderr, "Usage: %s [-v] [-c serverroot] [-d]\n", argv[0]);
#endif
			exit(1);
		}
	}
	...
	syslog(LOG_NOTICE,"%s,%i: SSL started", __FILE__, __LINE__);
#endif /*SERVER_SSL*/
...
#endif
	syslog(LOG_ERR, "fatal error %d", exit_code);
...
InitSSLStuff(void)
{
	syslog(LOG_DEBUG, "Entering InitSSLStuff");
#ifdef INET6
	...
	syslog(LOG_DEBUG, "%s,%i: About to load error strings\n", __FILE__,__LINE__);fflush(NULL);
/*	SSL_load_error_strings();*/
	SSLeay_add_ssl_algorithms();
	syslog(LOG_DEBUG, "%s,%i: Loaded error strings", __FILE__,__LINE__);fflush(NULL);
	...
	syslog(LOG_DEBUG,"SSL stuff inited okay");
	return 1;
}
#endif /*SERVER_SSL*/
