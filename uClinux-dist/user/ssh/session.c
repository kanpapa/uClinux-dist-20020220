...
#else
			syslog(LOG_ALERT,"WARNING WARNING");
			syslog(LOG_ALERT,"In embedded environments everybody becomes root");
			syslog(LOG_ALERT,"WARNING WARNING");
#endif /*EMBED*/
...
do_exec_pty(Session *s, const char *command, struct passwd * pw)
{
	...
	char line[256];
	const char *hostname;
	int fdout, tmp, ptyfd, ttyfd, ptymaster;
	int quiet_login;
	pid_t pid;
	socklen_t fromlen;
...
#else
	syslog(LOG_ALERT,"WARNING WARNING\n");
	syslog(LOG_ALERT,"Currently everything is run as root");
	syslog(LOG_ALERT,"in an embedded environment.");
	syslog(LOG_ALERT,"WARNING WARNING\n");
#endif /*EMBED*/
	/* Get window size from the packet. */
	pty_change_window_size(s->ptyfd, s->row, s->col, s->xpixel, s->ypixel);
